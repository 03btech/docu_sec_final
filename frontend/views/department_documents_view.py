...
class DepartmentDocumentsView(QWidget):
    ...
    def setup_ui(self):
        ...
        # Title
        title = QLabel("Department Documents")
        title.setStyleSheet("""
            QLabel {
                font-size: 28px;
                font-weight: bold;
        ...
        # Description
        description = QLabel("Documents shared within your department")
    ...
    def load_documents(self):
        try:
            self.documents = self.api_client.get_department_documents()
            print(f"DEBUG Department: Received {len(self.documents)} documents")
            if self.documents:
                print(f"DEBUG Department: First document: {self.documents[0]}")
            self.populate_table()
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to load department documents: {str(e)}")
    ...
    def view_document(self, row):
        ...
        if classification == 'confidential':
            # Warn user about security requirements
